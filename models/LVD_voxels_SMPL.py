...
class Model(BaseModel):
    ...
    def _create_img_encoder(self):
        return NetworksFactory.get_by_name('LVD_voxels', 2048, 6890*3, input_dim=7, b_min = np.array([-0.8, -0.8, -0.8]), b_max = np.array([0.8, 0.8, 0.8]))
    ...
    def get_current_visuals(self):
        ...
        trim_mesh = trimesh.Trimesh(self.pred_mesh, self.SMPL.faces, process=False)
        trim_mesh.visual.vertex_colors[:, :3] = [160, 160, 255]
        scene = pyrender.Scene(ambient_light=[.1, .1, .3], bg_color=(0, 0, 0))
        pyrender_mesh = pyrender.Mesh.from_trimesh(trim_mesh, smooth=False)
