...
class Model(BaseModel):
    def __init__(self, opt):
        ...
        #self.gaussian_sigma = 0.5
        self._reconstruction_error = torch.FloatTensor([0])
    ...
    def _create_img_encoder(self):
        return NetworksFactory.get_by_name('LVD_images', pred_dimensions=6890*3)
    ...
    def set_input(self, input):
        ...
        self._input_image = self._input_image[:,:3]
    ...
    def forward(self, keep_data_for_visuals=False, interpolate=0, resolution=128):
        ...
        if True:
            with torch.no_grad():
                ...
                factor = 110 # Aprox to meters
                self._reconstruction_error = torch.sqrt(((self._target_smpl[0] - input_points[0])**2).sum(-1)).mean()/factor
    ...
    def get_current_errors(self):
        loss_dict = OrderedDict([
                                 ...
                                 ('Val. SMPL reconstruction error', self._reconstruction_error.cpu().data.numpy()),
                                 ])
        return loss_dict
    ...
    def get_current_visuals(self):
        ...
        visuals = OrderedDict()
        visuals['input_image'] = (self._input_image[0].cpu().data.numpy().transpose(1,2,0)* [0.229, 0.224, 0.225]) + [0.485, 0.456, 0.406]
